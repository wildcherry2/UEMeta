...
namespace UEMeta::Testing {
    inline void ExpectDeclarationMetadata(const ParseResult::DeclarationMetadata& metadata, const std::string_view expected_name,
                                          const std::string_view expected_qualified_name, const std::filesystem::path& expected_file_path,
                                          const std::uint32_t expected_occurrence_index, const bool expected_is_anonymous) {
        ASSERT_TRUE(metadata.has_identifier());
        ExpectIdentifier(metadata.identifier(), expected_name, expected_qualified_name, expected_file_path);
