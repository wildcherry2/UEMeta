...
namespace {
    ...
    fs::path ForwardDeclarationSourcePath() { return fs::path{UEMETA_TEST_TARGET_INCLUDE_DIR} / "ForwardDeclarationTypes.hpp"; }
    ...
    const std::unordered_map<std::uint32_t, TLForwardDeclaration>& ForwardDeclarations() {
        static const auto declarations = [] {
            ...
            for (const auto& entry : fs::directory_iterator{fs::path{UEMETA_TEST_OUTPUT_DIR}}) {
                ...
                if (UEMeta::Testing::VersionedValue(declaration.metadata().identifier().file_path()) != ForwardDeclarationSourcePath().string()) {
                    continue;
                }

                result.insert_or_assign(UEMeta::Testing::VersionedValue(declaration.metadata().occurrence_index()), std::move(declaration));
            }
    ...
    const TLForwardDeclaration& ForwardDeclarationAt(const std::uint32_t occurrence_index) { return ForwardDeclarations().at(occurrence_index); }

    std::string ConcreteTemplateKey(const std::string_view template_name, const TemplateSpecializationKind specialization_kind) {
    ...
    const std::unordered_map<std::string, TLRecordDeclaration>& ConcreteTemplateDeclarations() {
        static const auto declarations = [] {
            ...
            for (const auto& entry : fs::directory_iterator{fs::path{UEMETA_TEST_OUTPUT_DIR}}) {
                ...
                if (!entry.is_regular_file() || (extension != ".classbin" && extension != ".structbin" && extension != ".unionbin")) {
                    continue;
                }
                ...
                const auto key =
                    ConcreteTemplateKey(declaration.metadata().identifier().name(), declaration.template_details().specialization_kind());
    ...
    void ExpectForwardDeclaration(const TLForwardDeclaration& declaration, const std::string_view expected_name,
                                  const std::uint32_t expected_occurrence_index, const ForwardDeclarationKind expected_kind,
        ...
        UEMeta::Testing::ExpectDeclarationMetadata(declaration.metadata(), expected_name, QualifiedName(expected_name),
                                                   ForwardDeclarationSourcePath(), expected_occurrence_index, false);
        EXPECT_EQ(declaration.kind(), expected_kind);
        EXPECT_EQ(UEMeta::Testing::VersionedValue(declaration.as_string()), expected_as_string);
    }

    void ExpectPrimaryTemplate(const TLForwardDeclaration& declaration, const std::string_view template_name, const std::string_view first_parameter,
        ...
        UEMeta::Testing::ExpectTemplateDetails(declaration.template_details(), TEMPLATE_SPECIALIZATION_NONE, qualified_template_name,
                                               ForwardDeclarationSourcePath(),
                                               {{first_parameter, qualified_template_name + "::" + std::string{first_parameter},
                                                 TEMPLATE_PARAMETER_KIND_TYPENAME, "typename " + std::string{first_parameter}},
                                                {second_parameter, qualified_template_name + "::" + std::string{second_parameter},
                                                 TEMPLATE_PARAMETER_KIND_TYPENAME, "typename " + std::string{second_parameter}}},
                                               R"()");
    }

    void ExpectExplicitPartialSpecialization(const TLForwardDeclaration& declaration, const std::string_view template_name,
                                             const std::string_view parameter_name) {
        ASSERT_TRUE(declaration.has_template_details());
        const auto qualified_template_name = QualifiedName(template_name);
        UEMeta::Testing::ExpectTemplateDetails(
            declaration.template_details(), TEMPLATE_SPECIALIZATION_EXPLICIT, qualified_template_name, ForwardDeclarationSourcePath(),
            {{parameter_name, qualified_template_name + "<type-parameter-0-0, int>::" + std::string{parameter_name}, TEMPLATE_PARAMETER_KIND_TYPENAME,
              "typename " + std::string{parameter_name}}},
            R"(type-parameter-0-0, int)");
    }

    void ExpectConcreteTemplateSpecialization(const std::string_view template_name, const std::string_view parameter_name,
                                              const TemplateSpecializationKind specialization_kind, const std::string_view arguments) {
        const auto& declaration = ConcreteTemplateDeclarations().at(ConcreteTemplateKey(template_name, specialization_kind));
        ASSERT_TRUE(declaration.has_template_details());

        const auto qualified_template_name = QualifiedName(template_name);
        UEMeta::Testing::ExpectTemplateDetails(declaration.template_details(), specialization_kind, qualified_template_name,
                                               ForwardDeclarationSourcePath(),
                                               {{parameter_name, qualified_template_name + "::" + std::string{parameter_name},
                                                 TEMPLATE_PARAMETER_KIND_TYPENAME, "typename " + std::string{parameter_name}}},
                                               arguments);
    }
} // namespace
...
TEST(ForwardDeclarationTests, UntemplatedClass) {
    ...
    ExpectForwardDeclaration(declaration, "ForwardClass", 0, FORWARD_DECLARATION_KIND_CLASS, R"(class ForwardClass)");
...
TEST(ForwardDeclarationTests, UntemplatedStruct) {
    ...
    ExpectForwardDeclaration(declaration, "ForwardStruct", 1, FORWARD_DECLARATION_KIND_STRUCT, R"(struct ForwardStruct)");
    EXPECT_FALSE(declaration.has_enum_details());
}

TEST(ForwardDeclarationTests, UntemplatedUnion) {
    const auto& declaration = ForwardDeclarationAt(2);
    ExpectForwardDeclaration(declaration, "ForwardUnion", 2, FORWARD_DECLARATION_KIND_UNION, R"(union ForwardUnion)");
    EXPECT_FALSE(declaration.has_enum_details());
}
...
TEST(ForwardDeclarationTests, UnscopedEnum) {
    ...
    ExpectForwardDeclaration(declaration, "ForwardUnscopedEnum", 9, FORWARD_DECLARATION_KIND_ENUM, R"(enum ForwardUnscopedEnum : int)");
    ASSERT_TRUE(declaration.has_enum_details());
    UEMeta::Testing::ExpectEnumDetails(declaration.enum_details(), ParseResult::ENUM_SCOPE_UNSCOPED, "int");
...
TEST(ForwardDeclarationTests, ScopedEnum) {
    ...
    ExpectForwardDeclaration(declaration, "ForwardScopedEnum", 10, FORWARD_DECLARATION_KIND_ENUM, R"(enum class ForwardScopedEnum : int)");
    ASSERT_TRUE(declaration.has_enum_details());
    UEMeta::Testing::ExpectEnumDetails(declaration.enum_details(), ParseResult::ENUM_SCOPE_CLASS, "int");
    EXPECT_FALSE(declaration.has_template_details());
}
...
TEST(ForwardDeclarationTests, ImplicitClassTemplateSpecialization) {
    ExpectConcreteTemplateSpecialization("ConcreteClassTemplate", "ConcreteClassType", TEMPLATE_SPECIALIZATION_IMPLICIT, R"(char)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationClassTemplateDeclaration) {
    ExpectConcreteTemplateSpecialization("ConcreteClassTemplate", "ConcreteClassType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DECLARATION,
                                         R"(short)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationClassTemplateDefinition) {
    ExpectConcreteTemplateSpecialization("ConcreteClassTemplate", "ConcreteClassType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DEFINITION,
                                         R"(long)");
}

TEST(ForwardDeclarationTests, ImplicitStructTemplateSpecialization) {
    ExpectConcreteTemplateSpecialization("ConcreteStructTemplate", "ConcreteStructType", TEMPLATE_SPECIALIZATION_IMPLICIT, R"(float)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationStructTemplateDeclaration) {
    ExpectConcreteTemplateSpecialization("ConcreteStructTemplate", "ConcreteStructType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DECLARATION,
                                         R"(double)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationStructTemplateDefinition) {
    ExpectConcreteTemplateSpecialization("ConcreteStructTemplate", "ConcreteStructType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DEFINITION,
                                         R"(long double)");
}

TEST(ForwardDeclarationTests, ImplicitUnionTemplateSpecialization) {
    ExpectConcreteTemplateSpecialization("ConcreteUnionTemplate", "ConcreteUnionType", TEMPLATE_SPECIALIZATION_IMPLICIT, R"(signed char)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationUnionTemplateDeclaration) {
    ExpectConcreteTemplateSpecialization("ConcreteUnionTemplate", "ConcreteUnionType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DECLARATION,
                                         R"(unsigned int)");
}

TEST(ForwardDeclarationTests, ExplicitInstantiationUnionTemplateDefinition) {
    ExpectConcreteTemplateSpecialization("ConcreteUnionTemplate", "ConcreteUnionType", TEMPLATE_SPECIALIZATION_EXPLICIT_INSTANTIATION_DEFINITION,
